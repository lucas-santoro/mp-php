...
$stmt = $pdo->prepare("SELECT status FROM transactions WHERE payment_id = :payment_id");
$stmt->execute([':payment_id' => $paymentId]);
$existingTransaction = $stmt->fetch(PDO::FETCH_ASSOC);

if ($existingTransaction && $existingTransaction['status'] === 'approved') 
{
    file_put_contents(__DIR__ . '/../../logs/webhook_error.log', date('Y-m-d H:i:s') . " - IGNORADO: Webhook duplicado para payment_id $paymentId.\n", FILE_APPEND);
